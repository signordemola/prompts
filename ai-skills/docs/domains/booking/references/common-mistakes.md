...
|---------|-------------|-----|
| `setHours()` in seed script | Wrong UTC timestamps, seeded appointments don't block slots | Use `londonDate()` from `lib/time.ts` |
...
| Seeding locally, deploying remotely | Timestamps use machine's local timezone, not London | Always seed on server or use `londonDate()` |
